...
@section('body')
@include('admin.band_material.particles.create_modal')

<div class="absolute overflow-auto overflow-x-hidden inset-0 p-4 bg-white sm:rounded-3xl shadow-lg">
    <div class="mb-6 flex justify-between">
        <p class="mb-2 text-gray-600 text-xl font-bold">Brand ({{count($bandMaterials)}} item)</p>
    ...
    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
        <table class="min-w-full table-fixed">
            ...
            <tbody class="bg-white">
                @foreach($bandMaterials as $bandMaterial)
                ...
                <tr class="hover:bg-gray-200 cursor-pointer text-sm font-medium text-gray-600" onclick="displayModal(true)">
                    ...
                    <td class="px-6 py-4 text-white">
                        <form class="inline" action="<!-- {{ route('band_material.destroy',$bandMaterial->id) }} -->" method="POST">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
