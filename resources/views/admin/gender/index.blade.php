...
@section('body')
@include('admin.gender.particles.create_modal')

<div class="absolute overflow-auto overflow-x-hidden inset-0 p-4 bg-white sm:rounded-3xl shadow-lg">
    <div class="mb-6 flex justify-between">
        <p class="mb-2 text-gray-600 text-xl font-bold">Gender ({{count($genders)}} item)</p>
            ...
            <tbody class="bg-white">
                ...
                <tr class="hover:bg-gray-200 cursor-pointer text-sm font-medium text-gray-600" onclick="displayModal(true)">
                    ...
                    <td class="px-6 py-4 text-white">
                        <form class="inline" action="<!-- {{ route('gender.destroy',$gender->id) }} -->" method="POST">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
